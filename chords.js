class ChordDetector {
  ...
  detect(activeNotes, midiNumbers = null) {
    ...
    pcs.forEach(rootPc => {
      const rootMidi = midiList.find(n => n % 12 === rootPc);
      ...
      for (const [name, formula] of Object.entries(this.chordFormulas)) {
        ...
        const alts = [];
        const hasMin3 = formula.includes(3);
        const hasMaj3 = formula.includes(4);
        ...
        const extCount  = [14,17,21].filter(iv => intervalsAbs.includes(iv)).length;
        ...
        // Construction du set réel des notes de l'accord détecté
        const chordPCs = new Set([
          ...formula,
          ...[14,17,21].filter(iv => intervalsAbs.includes(iv)), // extensions
          ...alts.map(a => {
            if (a === 'b9') return 13;
            if (a === '♯9') return 15;
            if (a === '♯11') return 18;
    ...
    // Tri avec priorité basse = fondamentale si même set de notes jouées
    results.sort((a, b) => {
      if (a.chordSignature === b.chordSignature) {
        const aBassRoot = a.root === a.bass ? 1 : 0;
        const bBassRoot = b.root === b.bass ? 1 : 0;
        if (bBassRoot !== aBassRoot) return bBassRoot - aBassRoot;
      }
    ...
    // Filtrage des sous-ensembles réels
    const filtered = [];
    const seenSets = [];
    ...
    for (const chord of results) {
      ...
      for (const biggerSig of seenSets) {
        const biggerArr = biggerSig.split(',').map(Number);
      ...
      if (!isSubset) {
        filtered.push(chord);
        seenSets.push(chord.chordSignature);
      }
    }
