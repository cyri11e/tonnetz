  ...
  detect(activeNotes) {
    ...
    pcs.forEach(rootPc => {
      ...
      for (const [name, formula] of Object.entries(this.chordFormulas)) {
        const matchCount = formula.filter(iv => intervals.includes(iv)).length;

        if (matchCount === formula.length) {
          results.push({
        ...
        } else if (matchCount >= 2 && pcs.length >= 3) {
          results.push({
            root: this.pcToNote[rootPc],
            type: name + ' (incomplet)',
            notes: pcs.map(pc => this.pcToNote[pc]),
    ...
    // Slash chords : basse ≠ fondamentale
    results.forEach(r => {
      if (r.bass !== r.root) {
        r.type += `/${r.bass}`;
      }
